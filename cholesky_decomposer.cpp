...
public:
    ...
    Eigen::MatrixXd solve(const Eigen::MatrixXd& b) const {
        if (llt.info() != Eigen::Success) {
            throw std::runtime_error("Decomposition is not valid for solving");
        }
        return llt.solve(b);
    }
...
PYBIND11_MODULE(cholesky_decomposer, m) {
    ...
    py::class_<CholeskyDecomposer>(m, "CholeskyDecomposer")
        ...
        .def("solve", &CholeskyDecomposer::solve, "Solve a linear system Ax = b using the Cholesky decomposition.");
}
